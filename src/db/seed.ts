...
/**
 * Seed script
 * Populates database with sample data in the correct order:
 * 1. Brands
 * 2. Categories
 * 3. SubCategories
 * 4. Segments
...
export async function seedDatabase() {
  ...
  const subCategoryMap = new Map(
    insertedSubCategories.map((sc) => [sc.name, sc.id])
  );
  ...
  const segmentMap = new Map(insertedSegments.map((s) => [s.name, s.id]));
  ...
  const skuMap = new Map(insertedSkus.map((s) => [s.skuCode, s.id]));
  ...
  const productData: NewProduct[] = [
    {
      ...
      skuId: skuMap.get("HGVPIN216")!,
      brandId: brandMap.get("High Garden")!,
      categoryId: categoryMap.get("Alcoholic Beverage")!,
      subCategoryId: subCategoryMap.get("Wine")!,
      segmentId: segmentMap.get("Red")!,
      globalWholesalePrice: 279.06,
      quantity: 45,
    },
    {
      title: "Koyama Methode Brut Nature NV",
      skuId: skuMap.get("KOYBRUNV6")!,
      brandId: brandMap.get("Koyama Wines")!,
      categoryId: categoryMap.get("Alcoholic Beverage")!,
      subCategoryId: subCategoryMap.get("Wine")!,
      segmentId: segmentMap.get("Sparkling")!,
      globalWholesalePrice: 120.0,
      quantity: 32,
    },
    {
      title: "Koyama Riesling 2018",
      skuId: skuMap.get("KOYNR1837")!,
      brandId: brandMap.get("Koyama Wines")!,
      categoryId: categoryMap.get("Alcoholic Beverage")!,
      subCategoryId: subCategoryMap.get("Wine")!,
      segmentId: segmentMap.get("Port/Dessert")!,
      globalWholesalePrice: 215.04,
      quantity: 28,
    },
    {
      title: "Koyama Tussock Riesling 2019",
      skuId: skuMap.get("KOYRIE19")!,
      brandId: brandMap.get("Koyama Wines")!,
      categoryId: categoryMap.get("Alcoholic Beverage")!,
      subCategoryId: subCategoryMap.get("Wine")!,
      segmentId: segmentMap.get("White")!,
      globalWholesalePrice: 215.04,
      quantity: 67,
    },
    {
      title: "Lacourte-Godbillon Brut Cru NV",
      skuId: skuMap.get("LACBNATNV6")!,
      brandId: brandMap.get("Lacourte-Godbillon")!,
      categoryId: categoryMap.get("Alcoholic Beverage")!,
      subCategoryId: subCategoryMap.get("Wine")!,
      segmentId: segmentMap.get("Sparkling")!,
